            ...
            for key in CREATION_DATE_KEYS {
                ...
                for tag_key in [*key, &key.to_uppercase()] {
                    if let Some(value) = tags.get(tag_key).and_then(|v| v.as_str()) {
                        if let Some(dt) = parse_video_datetime_utc(value) {
...
fn apply_timezone_correction(path: &Path, utc_time: NaiveDateTime) -> NaiveDateTime {
    ...
    if let Some(filename) = path.file_name().and_then(|f| f.to_str()) {
        if let Some(filename_time) = parse_filename_time(filename) {
...
/// Parse various video datetime formats and return as UTC
///
/// This function parses video metadata timestamps and returns them as UTC.
/// If the timestamp has a timezone, it's converted to UTC.
/// If no timezone is present, it's assumed to already be UTC (standard for MP4/MOV).
fn parse_video_datetime_utc(s: &str) -> Option<NaiveDateTime> {
    let s = s.trim();

    // Common video metadata formats
    let formats = [
        ...
        "%Y-%m-%dT%H:%M:%S%.f%z",
        "%Y-%m-%dT%H:%M:%S%z",
        // Without timezone (assumed UTC for video metadata)
        "%Y-%m-%dT%H:%M:%S%.f",
        "%Y-%m-%dT%H:%M:%S",
        "%Y-%m-%d %H:%M:%S%.f",
        "%Y-%m-%d %H:%M:%S",
    ...
    for format in formats {
        ...
        // Try parsing as NaiveDateTime (assumed UTC)
...
mod tests {
    ...
    #[test]
    fn test_parse_video_datetime_utc() {
        // ISO 8601 with Z
        let dt = parse_video_datetime_utc("2024-01-15T14:30:00Z").unwrap();
        assert_eq!(dt.year(), 2024);
        assert_eq!(dt.month(), 1);
        assert_eq!(dt.day(), 15);

        // With milliseconds
        let dt = parse_video_datetime_utc("2024-01-15T14:30:00.123Z").unwrap();
        assert_eq!(dt.year(), 2024);

        // With timezone offset - should convert to UTC
        let dt = parse_video_datetime_utc("2024-01-15T14:30:00+08:00").unwrap();
        assert_eq!(dt.year(), 2024);
        ...
        let dt = parse_video_datetime_utc("2024-01-15T14:30:00").unwrap();
        assert_eq!(dt.year(), 2024);

        // Space separator
        let dt = parse_video_datetime_utc("2024-01-15 14:30:00").unwrap();
        assert_eq!(dt.year(), 2024);

        // Invalid format
        assert!(parse_video_datetime_utc("invalid").is_none());
    }
}
